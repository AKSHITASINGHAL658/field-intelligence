...
export function SpecimenReveal({
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-4 lg:max-w-3xl animate-reveal-in">
      ...
      {/* Main Specimen Card — new discoveries get a one-shot celebratory
          glow; repeat observations stay calm so the signal stays meaningful */}
      <div
        className={`rounded-3xl bg-[#0C1015] border border-emerald-500/30 p-5 lg:p-6 shadow-2xl space-y-4 lg:space-y-0 lg:flex lg:flex-row lg:gap-6 lg:items-start ${
      ...
      >
        ...
        <div className="relative lg:w-2/5 lg:flex-shrink-0">
          ...
          {isNewDiscovery ? (
            <div className="absolute -bottom-3 left-1/2 bg-emerald-500 text-black px-3.5 py-1 rounded-full text-xs font-bold font-mono uppercase tracking-wider flex items-center gap-1.5 shadow-lg animate-reveal-pill-in">
              <Sparkles className="w-3.5 h-3.5" />
              NEW SPECIES DISCOVERED
            </div>
          ) : (
            <div className="absolute -bottom-3 left-1/2 bg-zinc-800 text-zinc-300 border border-zinc-700 px-3 py-0.5 rounded-full text-[10px] font-bold font-mono uppercase tracking-wider animate-reveal-pill-in">
              REPEAT OBSERVATION
            </div>
          )}
        </div>
        ...
        <div className="space-y-2 pt-1">
          ...
          <button
            ...
            className="w-full pt-2 flex items-center justify-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-300 transition-colors font-mono active:scale-[0.98]"
          >
